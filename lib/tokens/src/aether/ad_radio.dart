...
const double _kInnerRadius = 3.5;

class AdRadio<T> extends StatefulWidget {
  const AdRadio({
    super.key,
    required this.value,
  ...
  @override
  State<AdRadio<T>> createState() => _AdRadioState<T>();
}

class _AdRadioState<T> extends State<AdRadio<T>>
  ...
  @override
  void didUpdateWidget(AdRadio<T> oldWidget) {
  ...
  MaterialStateProperty<Color?> get _widgetFillColor {
    return MaterialStateProperty.resolveWith((Set<MaterialState> states) {
      if (states.contains(MaterialState.disabled)) {
        //return null;
        return adrColor.borderDisable;
        //return adrColor.textLink;
      }
      if (states.contains(MaterialState.selected)) {
        //TODO PASTIIN ACTIVECOLOR SUDAH WARNA BIRU ADR
...
class _RadioPainter extends ToggleablePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint paint = Paint()
      ...
      ..color = Color.lerp(adrColor.borderDisable, activeColor, position.value)!
    ...
    if (!position.isDismissed) {
      Set<MaterialState> disabledState = <MaterialState>{
      ...
      //TODO PAINT COLOR NYA CUMA AKTIF JIKA STATE NYA DISABLE
      //paint.color = adrColor.backgroundDisable;
      //paint.color = MaterialStateProperty.resolveAs<Color>(
      //  adrColor.textSuccess, disabledState);
